# routers/admin_users.py — v1.5.0 (13/08/2025)
# Administração de usuários (compat ampla + hardening) + Alterar Senha
# - Mantém tudo da v1.4.0 e adiciona:
#   * GET/POST /admin/users/{user_id}/senha  (tela e ação de alteração de senha)
# - Continua compat com múltiplos nomes de campos (email, admin, senha, created_at)
# - Evita AttributeError em propriedades sem setter (ex.: is_admin @property)
# - Verifica unicidade de username/email apenas se os campos existirem
# - Hash de senha reaproveita hashers do projeto, cai para sha256 se não houver
# - Helpers expostos ao template: get_email / is_user_admin
...
def set_admin_flags(user_obj: User, admin_flag: bool) -> None:
    ...
    1) Tenta via role/perfil/profile (strings "admin"/"user")
    2) Tenta via booleanos (is_admin/isAdmin/admin), protegido contra property sem setter
